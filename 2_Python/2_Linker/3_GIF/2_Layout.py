...
def addFile() :
    files = filedialog.askopenfilenames(title="Select Images",
                                        filetypes=(("All Files", "*.*"), ("JPEG File", "*.jpeg")),
                                        initialdir=path)
    for file in files:
...
def delFile() :
    for index in reversed(img_list.curselection()):
...
list_frame.pack(padx=5, pady=5, fill='both')    # fill='both' = fill x + y

scr = Scrollbar(list_frame)
scr.pack(pady=5, side='right', fill='y')

img_list = Listbox(list_frame, selectmode='extension', height=15, yscrollcommand=scr.set)       # 'extension' = multi-select    height = how many untill scroll
...
path_btn = Button(path_frame, text="Browse..", command=pathBrowse)      # Camel Case
...
cmb_width.current(0)    # what is currently in the width box
...
e_duration = Entry(opt_frame, width=10)
...
def run():
    ...
    # 2. Get maxes of width, height for canvas
    image_size = []
    ...
    if img_width < 0:
        image_size = [(image.size[0], image.size[1]) for image in images]       # [0] = width [1] = height
    else :
        img_width = int(img_width)
        image_size = [(int(img_width), (image.width * image.size[1] / image.size[0])) for image in images]
    ...
    for idx, img in enumerate(images):      # idx = index (can write anything) (ex. i, index ....)
        ...
        result_img.append(Image.new("RGB", (max_width, max_height), (0,0,0)))
        result_img_w, result_img_h = result_img[0].size

        offset = ((result_img_w - img_w) // 2, (result_img_h - img_h) // 2)
